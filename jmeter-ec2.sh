...
function runsetup() {
    ...
    for counter in ${!hosts[@]} ; do
        ...
        /usr/local/lib/$JMETER_VERSION/bin/jmeter.sh -n \
...
function runcleanup() {
    ...
    tail -n 1 $project_home/results/$project-$DATETIME-complete.jtl > $project_home/results/$NAME-${GIT_COMMIT:0:7}-$project-$DATETIME.jtl
    sed '$ d' $project_home/results/$project-$DATETIME-complete.jtl >> $project_home/results/$NAME-${GIT_COMMIT:0:7}-$project-$DATETIME.jtl
